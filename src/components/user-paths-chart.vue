...
function savePreset() {
  const suggestion = activePresetName.value || `Preset ${Object.keys(presets.value).length + 1}`
  const name = window.prompt('Tên preset:', suggestion)
  if (!name) return
  const trimmed = name.trim()
  if (!trimmed) return
  if (presets.value[trimmed] && !window.confirm(`Preset "${trimmed}" đã tồn tại. Ghi đè?`)) return
  presets.value = { ...presets.value, [trimmed]: Array.from(hiddenEvents.value) }
  persistPresets()
...
function parseJourneys(data, dedupFlag, hidden, endStep) {
  ...
  data.forEach((user, idx) => {
    const personId = user.person_id || user.distinct_id || user.user_id || `user-${idx}`
...
function buildGraph(journeys, max, top) {
  ...
  Object.keys(linkData).forEach((key) => {
    ...
    if (
      ...
      links.push({ step, source: src, target: tgt, value: count, userIds: ids })
    }
  })
...
const layout = computed(() => {
  const journeys = parseJourneys(props.rawData, dedup.value, hiddenEvents.value, endStepName.value)
  ...
  const totalPerStep = stepNodes.map((arr) => arr.reduce((s, n) => s + n.count, 0))
  const maxTotal = Math.max(...totalPerStep, 1)
  const pixelsPerUser = (AVAILABLE_HEIGHT - NODE_GAP * (topN.value - 1)) / maxTotal
...
const highlight = computed(() => {
  ...
  const startNode = layout.value.nodes.find((n) => `${n.stepIdx}|${n.name}` === startKey)
  ...
  return { activeNodes, activeLinks, upstreamNodes, upstreamLinks, activeUserIds }
})
...
const dropoffAlerts = computed(() => {
  ...
  function totalAtStep(idx) {
    return nodes.filter((n) => n.stepIdx === idx).reduce((s, n) => s + n.count, 0)
  }

  const alerts = []
  // Tính theo từng cột (step) — so total step với step trước
  const stepIndices = [...new Set(nodes.map((n) => n.stepIdx))].sort((a, b) => a - b)
...
const popoverStats = computed(() => {
  ...
  const pctFromStart = firstStepTotal > 0 ? (currentCount / firstStepTotal) * 100 : null
  const pctFromPrev = prevStepTotal > 0 ? (currentCount / prevStepTotal) * 100 : null
  const dropoffFromPrev = prevStepTotal > 0 ? prevStepTotal - currentCount : 0
  const dropoffPctFromPrev = prevStepTotal > 0 ? (dropoffFromPrev / prevStepTotal) * 100 : null
  ...
  if (stepIdx > 0 && prevStepTotal > 0) {
    ...
    leaversDropped = Array.from(prevUserIds).filter((id) => !currentStepUserIds.has(id)).length
    ...
    dropoffSourcesBreakdown = prevNodesAtStep
      .map((prevNode) => {
        const continuedIds = prevBreakdown.find((b) => b.source === prevNode.name)?.userIds || []
        ...
        return {
          ...
          continuedCount,
          droppedCount,
          dropPct: prevNode.count > 0 ? (droppedCount / prevNode.count) * 100 : 0,
        }
      })
      .filter((x) => x.droppedCount > 0)
      .sort((a, b) => b.droppedCount - a.droppedCount)
  ...
  if (lower.includes('thất bại') || lower.includes('error') || lower.includes('failed')) {
    insights.push('Event này biểu thị lỗi → drop-off cao là tự nhiên, nhưng cần giảm số lỗi.')
  }
  if (lower.includes('login') || lower.includes('đăng nhập') || lower.includes('signup') || lower.includes('đăng ký')) {
    insights.push('Bước auth thường rớt nhiều — kiểm tra form, social login, OTP delay.')
  }
  if (lower.includes('giá') || lower.includes('pricing') || lower.includes('payment') || lower.includes('thanh toán')) {
    insights.push('Bước giá / thanh toán nhạy cảm — xem giá, phương thức thanh toán, niềm tin.')
  }
  ...
  if (lower.includes('đồng bộ') || lower.includes('sync')) {
    insights.push('Đồng bộ chậm có thể khiến user bỏ — kiểm tra thời gian sync, có loading rõ ràng không.')
  }
...
const STRING_FORCED_KEYS = new Set([
  'business_id', 'person_id', 'distinct_id', 'user_id',
  'profile_fbid', 'fbid', 'session_id',
])
...
const nodePropertiesStats = computed(() => {
  ...
  const stats = Array.from(keysSet).map((key) => {
    const values = records.map((r) => r[key]).filter((v) => v != null && v !== '')
  ...
  const KEY_PRIORITY = ['business_id', 'distinct_id', 'person_id', 'user_id', 'profile_fbid', 'fbid', 'session_id']
  stats.sort((a, b) => {
...
function isLinkActive(g) {
  if (!activeNode.value) return null
  return highlight.value.activeLinks.has(`${g.link.step}|${g.link.source}|${g.link.target}`)
}

function onNodeEnter(node, e) {
  hoveredNode.value = { stepIdx: node.stepIdx, name: node.name }
  showTooltip(`<b>${fmtInt(node.count)}</b> user · Bước ${node.stepIdx + 1}<br>${node.name}`, e)
}
...
function focusAlert(alert) {
  ...
  if (nodes.length === 0) return
  const target = nodes.reduce((max, n) => (n.count > max.count ? n : max), nodes[0])
  popover.value = {
    visible: true,
    showList: false,
    x: 0,
...
function toggleUserDetailSort(key) {
  if (userDetailSort.value.key === key) {
    userDetailSort.value.dir = userDetailSort.value.dir === 'asc' ? 'desc' : 'asc'
  } else {
...
const userDetailTable = computed(() => {
  ...
  const activeFilters = userDetailFilters.value.filter((f) => f.key && keys.includes(f.key))
  const totalCount = rows.length
  let filteredRows = rows
  if (activeFilters.length > 0) {
    filteredRows = rows.filter((r) => activeFilters.every((f) => rowPassesFilter(r, f)))
  }
  ...
  return { rows: sortedRows, keys, totalCount, filteredCount: sortedRows.length, allRows: rows }
})
...
<template>
  <div class="upc-root">
    <div class="upc-controls">
      ...
      <label class="upc-control">
        Card width
        <input type="number" v-model.number="cardWidth" min="120" max="400" step="10" />
      </label>
      <label class="upc-control">
        Column gap
        <input type="number" v-model.number="columnGap" min="20" max="240" step="10" />
      </label>
      <button class="upc-customize-btn" @click="showCustomize = !showCustomize">
        ⚙ Tuỳ chỉnh
        <span v-if="hiddenEvents.size > 0" class="upc-customize-badge">{{ hiddenEvents.size }} ẩn</span>
      </button>
      <div class="upc-meta">
    ...
    <div v-if="showCustomize" class="upc-customize-panel">
      <div class="upc-customize-header">
        ...
        />
        <button class="upc-customize-action" @click="showAllEvents" :disabled="hiddenEvents.size === 0">
          Hiện tất cả
        </button>
        <button class="upc-customize-action" @click="hideAllFiltered">Ẩn kết quả lọc</button>
        <button class="upc-customize-close" @click="showCustomize = false">×</button>
      </div>
      ...
      <div class="upc-presets-bar">
        ...
        <select
          ...
          @change="(e) => e.target.value ? loadPreset(e.target.value) : setActivePreset('')"
        >
    ...
    <div v-if="props.loading" class="upc-analysis-banner upc-analysis-banner--loading">
      <div class="upc-analysis-banner-header">
        <div class="upc-spinner upc-spinner--sm"></div>
        <span class="upc-analysis-banner-title upc-analysis-banner-title--muted">
          Đang phân tích các bước rơi rụng…
        </span>
      </div>
      <div class="upc-analysis-banner-list">
        <div v-for="i in 3" :key="i" class="upc-analysis-card upc-analysis-card--skeleton">
    ...
    <div v-else-if="dropoffAlerts.length > 0" class="upc-analysis-banner">
      <div class="upc-analysis-banner-header">
        ...
        <span class="upc-analysis-banner-title">
          Phân tích nhanh: phát hiện <b>{{ dropoffAlerts.length }}</b> bước rơi rụng &gt; {{ DROPOFF_THRESHOLD }}%
        </span>
        <span class="upc-analysis-banner-hint">Click vào card để pin highlight luồng</span>
      </div>
      <div class="upc-analysis-banner-list">
        <div
        ...
        >
          ...
          <div class="upc-analysis-card-meta">
            {{ fmtInt(alert.dropped) }} / {{ fmtInt(alert.prevTotal) }} user rời bỏ
          </div>
          <div
          ...
          >
            ...
            >
              ...
              <span class="upc-analysis-card-source-num">{{ fmtInt(s.droppedCount) }} ({{ fmtPct(s.dropPct) }})</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="upc-chart-wrap" :class="{ 'upc-chart-wrap--loading': props.loading }">
        ...
        <g>
          ...
          >
            <rect
              ...
              :class="{
                'upc-card-bg--active':
                  popover.visible && popover.stepIdx === n.stepIdx && popover.name === n.name,
              }"
            />
            <rect
              :x="n.x + n.w + 16"
            ...
            <text
              ...
              text-anchor="middle"
              class="upc-step-text"
            >{{ String(n.stepIdx + 1).padStart(2, '0') }}</text>
    ...
    >
      ...
      >
        <div class="upc-pop-title">
          <span class="upc-pop-badge">{{ String(popover.stepIdx + 1).padStart(2, '0') }}</span>
          <span class="upc-pop-name">{{ popover.name }}</span>
        </div>
        <button class="upc-pop-close" @click="closePopover" aria-label="Đóng">×</button>
      </div>
      ...
      <div v-if="popoverStats" class="upc-pop-stats">
        <div class="upc-pop-stat-row">
          <div class="upc-pop-stat">
            ...
            <div class="upc-pop-stat-value">{{ fmtInt(popoverStats.currentCount) }}</div>
          </div>
          <div class="upc-pop-stat" v-if="popover.stepIdx > 0">
            ...
            <div class="upc-pop-stat-value">
              ...
              <span class="upc-pop-stat-sub">/ {{ fmtInt(popoverStats.prevStepTotal) }}</span>
            </div>
          </div>
          <div class="upc-pop-stat" v-if="popover.stepIdx > 0">
            ...
            <div class="upc-pop-stat-value">
              ...
              <span class="upc-pop-stat-sub">/ {{ fmtInt(popoverStats.firstStepTotal) }}</span>
            </div>
          </div>
          <div
            ...
            :style="{
              borderColor: dropoffColor(popoverStats.dropoffPctFromPrev)?.border,
          ...
          >
            ...
            <div
              ...
              :style="{ color: dropoffColor(popoverStats.dropoffPctFromPrev)?.text }"
            >
              {{ fmtPct(popoverStats.dropoffPctFromPrev) }}
              <span class="upc-pop-stat-sub">/ {{ fmtInt(popoverStats.dropoffFromPrev) }} user</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="nodePropertiesStats" class="upc-pop-props">
          <div class="upc-pop-props-title">
            📋 Chỉ số ở bước này
            <span class="upc-pop-props-meta">({{ fmtInt(nodePropertiesStats.recordCount) }}/{{ fmtInt(popover.userIds.length) }} user có data)</span>
          </div>
            ...
            >
              ...
              <template v-else-if="s.kind === 'string'">
                <div
                  ...
                  :title="t.value"
                >
                  <span class="upc-pop-prop-dist-bar" :style="{ width: t.pct + '%' }"></span>
                  <span class="upc-pop-prop-dist-val">{{ t.value }}</span>
                  <span class="upc-pop-prop-dist-num">{{ fmtInt(t.count) }} · {{ fmtPct(t.pct) }}</span>
                </div>
              </template>
            </div>
          </div>
        ...
        <div class="upc-pop-action-row">
          ...
          >🎯 Đặt làm End step</button>
          <button
            v-else
            class="upc-pop-action-btn upc-pop-action-btn--active"
            @click="clearEndStep"
          >✓ Đang là End step · Bỏ</button>
          <button
            v-if="popover.stepIdx > 0 && popoverStats.dropoffPctFromPrev != null"
            class="upc-pop-action-btn"
            :class="{ 'upc-pop-action-btn--alert': popoverStats.dropoffPctFromPrev > 40 }"
          ...
          >📋 Chi tiết user</button>
        </div>

        <div v-if="prevBreakdownArcs.length > 0" class="upc-pop-prev">
          <div class="upc-pop-prev-title">User đến từ (bước {{ popover.stepIdx }})</div>
          <div class="upc-pop-donut-wrap">
            <!-- Donut SVG -->
            <svg class="upc-pop-donut" viewBox="0 0 80 80" width="100" height="100">
              <!-- Background ring (nhạt) -->
              <circle cx="40" cy="40" r="32" fill="none" stroke="#f3f4f6" stroke-width="14" />
              ...
              <g transform="rotate(-90 40 40)">
                <circle
                  ...
                  :class="['upc-pop-donut-slice', { 'upc-pop-donut-slice--dim': hoveredSource && hoveredSource !== arc.source }]"
                  @mouseenter="hoveredDonutSource = arc.source"
                  @mouseleave="hoveredDonutSource = null"
                />
              </g>
              <!-- Center label: tổng user -->
              <text x="40" y="38" text-anchor="middle" class="upc-pop-donut-total">
                {{ fmtInt(popoverStats.currentCount) }}
              </text>
              <text x="40" y="50" text-anchor="middle" class="upc-pop-donut-sub">user</text>
            </svg>
            ...
            <div class="upc-pop-donut-legend">
              <div
                ...
                :class="{ 'upc-pop-legend-row--active': hoveredSource === arc.source }"
                :title="arc.source"
                @mouseenter="hoveredDonutSource = arc.source"
                @mouseleave="hoveredDonutSource = null"
              >
                <span class="upc-pop-legend-dot" :style="{ background: arc.color }"></span>
                <span class="upc-pop-legend-name">{{ arc.source }}</span>
                <span class="upc-pop-legend-num">{{ fmtInt(arc.count) }} · {{ fmtPct(arc.pct) }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="upc-pop-toolbar">
        ...
        >
          {{ copiedAll ? 'Đã copy' : `Copy all (${fmtInt(filteredUserIds.length)})` }}
        </button>
      </div>
      <div class="upc-pop-list">
        <div v-if="filteredUserIds.length === 0" class="upc-pop-empty">Không có user khớp</div>
        <div
    ...
    >
      <div class="upc-analysis-modal">
        <div class="upc-analysis-modal-header">
          ...
          <button class="upc-pop-close" @click="showAnalysisModal = false">×</button>
        </div>
        ...
        <div class="upc-analysis-modal-body">
          <div class="upc-pop-analysis-line">
            ...
            user từ <b>bước {{ popover.stepIdx }}</b> đã không vào "<b>{{ popover.name }}</b>"
            ({{ fmtPct(popoverStats.dropoffPctFromPrev) }}).
          </div>

          <div v-if="popoverStats.leaversOtherBranches.length > 0" class="upc-pop-analysis-section">
            <div class="upc-pop-analysis-section-title">↗ Họ chuyển sang nhánh khác:</div>
            <div
              ...
              :title="b.target"
            >
              <span class="upc-pop-analysis-bar" :style="{ width: Math.min(100, b.pct) + '%' }"></span>
              <span class="upc-pop-analysis-name">{{ b.target }}</span>
              <span class="upc-pop-analysis-meta">{{ fmtInt(b.count) }} user</span>
            </div>
          </div>

          <div v-if="popoverStats.leaversDropped > 0" class="upc-pop-analysis-section">
            <div class="upc-pop-analysis-section-title">⛔ Dừng hẳn (không có event tiếp theo):</div>
            <div class="upc-pop-analysis-row upc-pop-analysis-row--danger">
              <span class="upc-pop-analysis-name">User rời bỏ hành trình</span>
              <span class="upc-pop-analysis-meta">{{ fmtInt(popoverStats.leaversDropped) }} user</span>
            </div>
          </div>
          ...
          >
            <div class="upc-pop-analysis-section-title">🔍 Nguồn drop-off lớn nhất:</div>
            <div
            ...
            >
              ...
              <span class="upc-pop-analysis-meta">
                {{ fmtInt(s.droppedCount) }}/{{ fmtInt(s.totalAtSource) }} rớt ({{ fmtPct(s.dropPct) }})
              </span>
            </div>
          </div>

          <div v-if="popoverStats.insights.length > 0" class="upc-pop-analysis-section">
            <div class="upc-pop-analysis-section-title">💡 Gợi ý:</div>
            <ul class="upc-pop-analysis-insights">
              <li v-for="(ins, i) in popoverStats.insights" :key="i">{{ ins }}</li>
            </ul>
          </div>
        </div>
      </div>
    ...
    >
      <div class="upc-analysis-modal upc-user-detail-modal">
        <div class="upc-analysis-modal-header">
          <div>
            <div class="upc-analysis-modal-title">📋 Chi tiết user × property</div>
            <div class="upc-analysis-modal-sub">
              Bước {{ popover.stepIdx + 1 }} · {{ popover.name }} ·
          ...
          <button class="upc-pop-close" @click="showUserDetailModal = false">×</button>
        </div>
        ...
        <div class="upc-udt-filter-bar">
          <div class="upc-udt-filter-head">
            ...
            <button class="upc-udt-filter-add" @click="addUserDetailFilter">+ Thêm điều kiện</button>
            <button
          ...
          <div v-if="userDetailFilters.length > 0" class="upc-udt-filter-list">
            ...
            >
              <select v-model="f.key" class="upc-udt-filter-select">
                ...
                <option v-for="k in userDetailTable.keys" :key="k" :value="k">{{ k }}</option>
              </select>
              <select v-model="f.op" class="upc-udt-filter-op">
                ...
                <optgroup v-else label="Chuỗi">
                  <option v-for="op in STRING_OPS" :key="op.value" :value="op.value">{{ op.label }}</option>
                </optgroup>
                <optgroup label="Khác">
                  <option v-for="op in EMPTY_OPS" :key="op.value" :value="op.value">{{ op.label }}</option>
                </optgroup>
              </select>
              <input
        ...
        <div v-if="userDetailTable.rows.length === 0" class="upc-udt-empty-state">
          Không có user nào khớp với bộ lọc.
          <button class="upc-udt-filter-clear" @click="clearUserDetailFilters">Xoá bộ lọc</button>
        </div>
          ...
          <table class="upc-user-detail-table">
            ...
            <tbody>
              <tr v-for="row in userDetailTable.rows" :key="row.personId">
                <td class="upc-udt-id" :title="row.personId">{{ row.personId }}</td>
                <td
                  v-for="key in userDetailTable.keys"
                  :key="key"
                  class="upc-udt-cell"
                  :class="{ 'upc-udt-cell--num': typeof row.props?.[key] === 'number' || Number.isFinite(Number(row.props?.[key])) }"
                >
...
.upc-customize-btn:hover { background: #f9fafb; border-color: #9ca3af; }
.upc-customize-badge {
...
.upc-customize-search:focus { outline: none; border-color: #6366f1; }
...
.upc-customize-action:hover:not(:disabled) { background: #f3f4f6; }
.upc-customize-action:disabled { opacity: 0.5; cursor: not-allowed; }
.upc-customize-action--danger {
  color: #991b1b; border-color: #fecaca;
}
.upc-customize-action--danger:hover { background: #fef2f2; }
...
.upc-presets-select:focus { outline: none; border-color: #6366f1; }
.upc-presets-info {
  margin-left: auto;
...
.upc-customize-close:hover { color: #111827; }
...
.upc-customize-item:hover { background: #f3f4f6; }
.upc-customize-item input { cursor: pointer; }
.upc-customize-name {
  flex: 1; min-width: 0;
...
.upc-customize-empty {
  padding: 16px; color: #9ca3af; font-size: 12px;
  text-align: center;
}
/* Banner End step */
.upc-endstep-banner {
...
.upc-endstep-icon { font-size: 16px; }
.upc-endstep-label { font-weight: 600; }
.upc-endstep-name {
  font-weight: 700;
  background: #fff;
...
.upc-endstep-clear:hover { background: #e0e7ff; }
...
.upc-analysis-banner-icon { font-size: 16px; }
.upc-analysis-banner-title {
  font-size: 13px; font-weight: 600;
  color: #92400e;
}
.upc-analysis-banner-title b { color: #b91c1c; }
...
.upc-chart-wrap {
  overflow-x: auto; overflow-y: hidden; background: #fafbfc;
  position: relative;
  min-height: 200px;
}
.upc-chart-wrap--loading svg { opacity: 0.3; pointer-events: none; }
...
.upc-loading-text { font-size: 13px; color: #4338ca; font-weight: 500; }
.upc-spinner {
...
.upc-analysis-banner-title--muted { color: #6b7280; font-weight: 500; }
...
.upc-skeleton-bar--short { width: 40%; }
.upc-skeleton-bar--medium { width: 70%; }
@keyframes upc-skeleton-shimmer {
  0% { background-position: 200% 0; }
  100% { background-position: -200% 0; }
}
.upc-bar { fill: #6366f1; cursor: pointer; transition: fill 0.15s, opacity 0.15s; }
.upc-bar--active { fill: #4338ca; }
.upc-bar--upstream { fill: #818cf8; opacity: 0.7; }
.upc-bar--dim { opacity: 0.2; }

.upc-link {
  fill: #6366f1; fill-opacity: 0.25; transition: fill-opacity 0.15s, fill 0.15s;
}
.upc-link:hover { fill-opacity: 0.55; }
.upc-link--active { fill: #4338ca; fill-opacity: 0.65; }
.upc-link--upstream { fill: #818cf8; fill-opacity: 0.3; }
.upc-link--dim { fill-opacity: 0.04; }

.upc-card { cursor: pointer; transition: opacity 0.15s; }
.upc-card--dim { opacity: 0.3; }
.upc-card-bg { fill: #fff; stroke: #e5e7eb; stroke-width: 1; transition: stroke 0.12s, fill 0.12s; }
.upc-card:hover .upc-card-bg { stroke: #6366f1; stroke-width: 1.5; }
.upc-card--active .upc-card-bg { stroke: #4338ca; stroke-width: 2; fill: #eef2ff; }
.upc-card--upstream .upc-card-bg { stroke: #c7d2fe; stroke-width: 1.5; fill: #f5f3ff; }
.upc-card-bg--active { stroke: #4f46e5; stroke-width: 2; }
.upc-step-badge { fill: #f3f4f6; }
.upc-step-text { fill: #6b7280; font-size: 11px; font-weight: 600; }
.upc-label { fill: #111827; font-size: 12px; font-weight: 500; }
.upc-count { fill: #111827; font-size: 12px; font-weight: 600; }
</style>
...
.upc-pop-body > * { margin-top: 0 !important; }
.upc-pop-header {
...
.upc-pop-header--dragging { cursor: grabbing; }
.upc-pop-title {
  display: flex; align-items: center; gap: 8px;
  min-width: 0;
}
...
.upc-pop-close:hover { color: #111827; }
...
.upc-pop-stat--danger { border-color: #fecaca; background: #fef2f2; }
.upc-pop-stat-label {
  font-size: 10px; font-weight: 600;
...
.upc-pop-stat--danger .upc-pop-stat-value { color: #991b1b; }
.upc-pop-stat-sub {
...
.upc-pop-prop--wide { grid-column: span 2; }
.upc-pop-prop-key {
  font-size: 10px; font-weight: 600;
  color: #6b7280;
  text-transform: uppercase;
...
.upc-pop-prop-label { font-size: 10px; color: #9ca3af; }
.upc-pop-prop-value {
  font-size: 14px; font-weight: 700;
  color: #4338ca;
  font-variant-numeric: tabular-nums;
...
.upc-pop-prop-row b { color: #111827; }
...
.upc-user-detail-table thead th:hover { background: #f3f4f6; }
.upc-udt-sort { color: #6366f1; font-size: 10px; margin-left: 4px; }
.upc-udt-id-col { min-width: 280px; }
.upc-udt-prop-col { min-width: 120px; }
.upc-udt-action-col { width: 90px; cursor: default !important; }
.upc-udt-action-col:hover { background: #f9fafb !important; }
...
.upc-user-detail-table tbody tr:hover { background: #fafbfc; }
.upc-udt-id {
...
.upc-udt-filter-add:hover { background: #eef2ff; }
...
.upc-udt-filter-clear:hover { background: #fef2f2; }
.upc-udt-filter-list {
  display: flex; flex-direction: column;
  gap: 6px;
}
...
.upc-udt-filter-select { min-width: 160px; }
.upc-udt-filter-op { min-width: 110px; }
.upc-udt-filter-input { flex: 1; min-width: 140px; }
...
.upc-pop-analysis-icon { font-size: 16px; }
.upc-pop-analysis-title {
  font-size: 12px; font-weight: 600;
  color: #92400e;
}
.upc-pop-analysis-title b { color: #b91c1c; }
...
.upc-pop-analysis-row--danger { background: #fee2e2; }
.upc-pop-analysis-row--danger .upc-pop-analysis-name,
.upc-pop-analysis-row--danger .upc-pop-analysis-meta { color: #991b1b; }
...
.upc-pop-analysis-insights li { margin-bottom: 2px; }
...
.upc-pop-prev-row--active .upc-pop-prev-bar { background: #c7d2fe; }
.upc-pop-prev-row--active .upc-pop-prev-name { color: #4338ca; font-weight: 600; }
.upc-pop-prev-bar {
  position: absolute;
  top: 0; left: 0; bottom: 0;
  background: #eef2ff;
...
.upc-pop-donut { flex-shrink: 0; }
.upc-pop-donut-slice {
  transition: opacity 0.15s;
  cursor: pointer;
}
.upc-pop-donut-slice--dim { opacity: 0.25; }
.upc-pop-donut-total {
  font-size: 14px; font-weight: 700; fill: #111827;
...
.upc-pop-legend-row:hover { background: #f9fafb; }
...
.upc-pop-search:focus { outline: none; border-color: #6366f1; }
.upc-pop-btn {
  padding: 5px 10px;
  border: 1px solid #d1d5db;
  background: #fff;
...
.upc-pop-btn:hover:not(:disabled) { background: #f9fafb; }
.upc-pop-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.upc-pop-btn--ok { background: #ecfdf5; border-color: #10b981; color: #065f46; }
...
.upc-pop-row:hover { background: #f9fafb; }
.upc-pop-id {
  font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace;
  font-size: 11.5px;
  color: #374151;
...
.upc-pop-row:hover .upc-pop-actions { opacity: 1; }
.upc-pop-link {
  font-size: 11px;
  color: #6366f1;
...
.upc-pop-link:hover { text-decoration: underline; color: #4f46e5; }
.upc-pop-copy {
  font-size: 11px; color: #6b7280;
  cursor: pointer;
}
.upc-pop-copy:hover { color: #4f46e5; }
.upc-pop-copy--ok { color: #059669 !important; opacity: 1 !important; }
.upc-pop-empty {
  padding: 16px 12px; color: #9ca3af; font-size: 12px;
  text-align: center;
}
</style>
